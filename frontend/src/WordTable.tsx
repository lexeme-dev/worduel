                ...
                { (idx + 1 == guessNumber) ? 
                  ( <Row key={-1}>
                    <Form className="join-form">
                      <Form.Group>
                        <Form.Control className="join-enter" type="text" size="lg" placeholder="guess" onChange={(e) => this.setState({guess: e.target.value})} />
                        <div className="d-grid gap-2">
                          <Button variant="primary join-button" onClick={() => this.props.onGuess(this.state.guess)}> GUESS </Button>
                        </div>
                      </Form.Group>
                    </Form>
                  </Row>) :
